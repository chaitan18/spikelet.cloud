...
export class AdminSubjectComponent implements OnInit {
  ...
  OnSubmit(form: NgForm){
    ...
    let adderror=1;
    if (form.value.subjectname == "") {
      this.addsubjecterrorMsg = "Please Enter The Subject Name";
      adderror=0;
    }else{
      this.addsubjecterrorMsg="";
    }
    if(adderror ==0){
      return;
    }
    ...
    //console.log("form.value==========="+JSON.stringify(tpoicid));
    const createtopic ={
              "userroleflag":"admin",
              "admincheckval": {
              "FilterExpression":"id = :id and refreshToken = :refreshToken", 
  ...
  OnEditSubmit(form: NgForm){
    ...
    const createtopic ={
                          ...
                          "payload": {
                          "Key":{
                              "subjectId": this.editdetails.subjectId
                          },
                          "UpdateExpression": "set subjectname = :subjectname",
                          "ExpressionAttributeValues":{
                              ":subjectname":form.value.upsubjectname
                          },
  ...
  deleteItem(){
   ...
   triggerFalseClick() {
    ...
    OnSearchSubmit(form: NgForm){
      let topic_name =form.value.ssubjectname;
      let infotopics;
      let session_details = localStorage.getItem('login_session_details');
      this.access_details = $.parseJSON(session_details);
      if(topic_name) {
        ...
        //   }
          ...
          infotopics ={
            ...
            "method": "list",
            "payload": {
              "FilterExpression": "contains (subjectname,:t) and adminId = :adminId",
